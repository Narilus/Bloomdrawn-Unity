...
namespace Bloomdrawn.Editor.Tooling
{
    public static class CombatStageAuthoring
    {
        ...
        public static void CreateOrUpdate()
        {
            ...
            var root = new GameObject("Combat Stage", typeof(CombatStageLayout));
            ...
            party.GetComponent<PartyFormationView>().Configure(CreateActors(party.transform, "party", 4)); enemies.GetComponent<EnemyFormationView>().Configure(CreateActors(enemies.transform, "enemy", 1));
            var survival=Region(canvas.transform,"Shared Survival Lane",new Vector2(.18f,.25f),new Vector2(.42f,.36f)); Region(canvas.transform,"Mana Region",new Vector2(.03f,.05f),new Vector2(.18f,.16f)); var hand=Region(canvas.transform,"Hand Safe Area",new Vector2(.25f,.01f),new Vector2(.72f,.25f)); var endTurn=Region(canvas.transform,"End Turn Control",new Vector2(.78f,.06f),new Vector2(.94f,.16f)); var log=Region(canvas.transform,"Combat Log Overlay",new Vector2(.72f,.68f),new Vector2(.96f,.93f)); var enemyLane=Region(canvas.transform,"Enemy Target Lane",new Vector2(.62f,.25f),new Vector2(.96f,.68f)); root.GetComponent<CombatStageLayout>().Configure(hand,survival,enemyLane,endTurn,log);
